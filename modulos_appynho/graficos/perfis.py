...
class plotagem:
    ...
    def plot_rodrigao(self,indice,dicio,codigo_lito,relacao_cor,padrao_local = False):
        ...
        std_0,std_1 = [],[]
        for i in range(len(dicio[j][2])):
            std_0.append(mean[i]-dicio[j][2][i])
            std_1.append(mean[i]+dicio[j][2][i])
            
        min_max_values = self.max_min_locais(std_0+std_1,prof,padrao)
        minx = min_max_values[0]
        maxx = min_max_values[1]
        miny = min_max_values[2]
        maxy = min_max_values[3]
        ...
        lab_s = self.ax[indice].plot(mean,prof,c = relacao_cor[codigo_lito][0],
                                     marker='o',
                                     label = padrao['linha_nome'],
                                     markersize=padrao['marcador_tamanho'],
                                     linewidth = 0.0)
        
        for i in range(len(std_0)):
            self.ax[indice].plot([std_0[i],std_1[i]],[prof[i],prof[i]],'-k',linewidth = padrao['linha_espessura'])
